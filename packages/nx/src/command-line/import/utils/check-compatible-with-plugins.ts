...
function findPluginAndFilesWithError(
  ...
  if (isAggregateCreateNodesError(error)) {
    pluginName = (error as AggregateCreateNodesError).pluginName;
    excludeFiles = (error as AggregateCreateNodesError).errors?.map(
      (error) => error?.[0]
    );
  } else if (isMergeNodesError(error)) {
    pluginName = (error as MergeNodesError).pluginName;
    excludeFiles = [(error as MergeNodesError).file];
  }
...
function findPluginWithName(
  ...
  let foundIndex = -1;
  let foundPlugin = plugins.find((plugin, index) => {
...
export function updatePluginsInNxJson(
  ...
  Object.entries(pluginNametoExcludeFiles).forEach(
    ([pluginName, excludeFiles]) => {
      ...
      output.warn({
        title: `Plugin ${pluginName} is incompatible`,
        bodyLines: [
